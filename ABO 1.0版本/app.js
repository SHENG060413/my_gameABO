...
function renderMapEventChoiceButton(location, eventDefinition, choice, disabled, kind) {
  ...
  return `
    <button class="button map-action-button" type="button" data-map-event-kind="${escapeHtml(kind)}" data-map-location-id="${escapeHtml(location.id)}" data-map-event-id="${escapeHtml(eventDefinition.id)}" data-map-event-choice="${escapeHtml(choice.id)}" ${disabled ? "disabled" : ""} aria-label="在${escapeHtml(location.name)}执行${escapeHtml(eventDefinition.title)}的${escapeHtml(choice.label)}，消耗 1 点行动力">
      <span>${escapeHtml(choice.label)}${detailText ? `<small>${escapeHtml(detailText)}</small>` : ""}</span>
      <span>耗费 1</span>
    </button>
  `;
}
...
function renderLifeGoalStoryDialog(goalId) {
  ...
  elements.lifeGoalStoryChoices.innerHTML = story.choices.map((choice) => {
    ...
    return `
      <button class="button age-story-choice" type="button" data-life-goal-story-choice="${escapeHtml(choice.id)}" aria-label="选择${escapeHtml(choice.label)}，${escapeHtml(effectText)}">
...
function renderSaveSlotSummaries() {
  elements.saveSlotList.querySelectorAll("[data-save-slot]").forEach((button) => {
    ...
    const summary = getSaveSlotSummary(slot, readSaveSlot(slot));
...
function selectSaveSlot(slot) {
  ...
  if (!isValidSaveSlot(nextSlot)) {
    setSaveLoadStatus("槽位选择失败：请选择 1-3 号本地存档槽。");
    return;
  }

  selectedSaveSlot = nextSlot;
  renderSaveSlotSummaries();
  setSaveLoadStatus(`已选择槽位 ${selectedSaveSlot}。保存与读取都会使用该槽位。`);
}
...
function saveCurrentLife() {
  ...
  if (!saveKey) {
    setSaveLoadStatus("保存失败：当前槽位无效，请选择 1-3 号本地存档槽。");
    return;
  }

  try {
    localStorage.setItem(saveKey, JSON.stringify(createSavePayload()));
    renderSaveSlotSummaries();
...
function autoSaveCurrentLife(reason) {
  ...
  const saveKey = getSaveSlotKey(selectedSaveSlot);
  if (!saveKey) {
    const message = "自动存档失败：当前槽位无效，请选择 1-3 号本地存档槽。";
    setSaveLoadStatus(message);
    return message;
  }
...
function renderAgeStoryDialog(age, story) {
  ...
  elements.ageStoryChoices.innerHTML = story.choices.map((choice) => `
    <button class="button age-story-choice" type="button" data-age-story-choice="${choice.id}" aria-label="选择${getAgeStoryChoiceLabel(choice)}，${choice.effectText}">
      <span>${getAgeStoryChoiceLabel(choice)}</span>
...
function openQuarterStoryDialog(age, quarter) {
  ...
  elements.quarterStoryChoices.innerHTML = story.choices.map((choice) => `
    <button class="button age-story-choice" type="button" data-quarter-story-choice="${choice.id}" aria-label="\u9009\u62e9${choice.label}\uff0c${formatChoiceEffectText(choice)}">
      <span>${choice.label}</span>
      <small>${formatChoiceEffectText(choice)}</small>
    </button>
  `).join("");
...
function renderAttributeThresholdStoryDialog(familyData, story) {
  ...
  elements.attributeThresholdStoryChoices.innerHTML = story.choices.map((choice) => `
    <button class="button age-story-choice" type="button" data-attribute-threshold-choice="${choice.id}" aria-label="选择${choice.label}，${formatChoiceEffectText(choice)}">
      <span>${choice.label}</span>
      <small>${formatChoiceEffectText(choice)}</small>
    </button>
  `).join("");
...
function renderSpecialActionDialog() {
  ...
  elements.specialActionChoices.innerHTML = node.choices.map((choice) => `
    <button class="button age-story-choice" type="button" data-special-action-choice="${choice.id}" aria-label="\u9009\u62e9${choice.label}\uff0c${formatChoiceEffectText(choice)}">
      <span>${choice.label}</span>
      <small>${formatChoiceEffectText(choice)}</small>
    </button>
  `).join("");
...
function renderRandomNpcEncounterDialog(encounter) {
  ...
  elements.randomNpcChoices.innerHTML = encounter.choices.map((choice) => `
    <button class="button age-story-choice" type="button" data-random-npc-choice="${escapeHtml(choice.id)}" aria-label="选择${escapeHtml(choice.label)}，${escapeHtml(formatChoiceEffectText(choice))}">
      <span>${escapeHtml(choice.label)}</span>
      <small>${escapeHtml(formatChoiceEffectText(choice))}</small>
    </button>
  `).join("");
}
...
function renderRelationshipStageEventDialog(stageEvent) {
  ...
  elements.relationshipStageEventChoices.innerHTML = definition.choices.map((choice) => {
    ...
    return `
      <button class="button age-story-choice" type="button" data-relationship-stage-choice="${escapeHtml(choice.id)}" aria-label="选择${escapeHtml(choice.label)}，${escapeHtml(effectText)}">
        <span>${escapeHtml(choice.label)}</span>
        <small>${escapeHtml(effectText)}</small>
      </button>
    `;
...
function renderProactiveNpcEventDialog(proactiveEvent) {
  ...
  elements.proactiveNpcEventChoices.innerHTML = definition.choices.map((choice) => {
    const effectText = formatRelationshipEffectText(choice.relationshipEffects || {}) || "关系：无变化";
    return `
      <button class="button age-story-choice" type="button" data-proactive-npc-choice="${escapeHtml(choice.id)}" aria-label="选择${escapeHtml(choice.label)}，${escapeHtml(effectText)}">
        <span>${escapeHtml(choice.label)}</span>
        <small>${escapeHtml(effectText)}</small>
      </button>
    `;
...
function renderNarrativeDiceEventDialog(diceEvent) {
  ...
  elements.narrativeDiceChoices.innerHTML = diceEvent.choices.map((choice) => {
    ...
    return `
      <button class="button age-story-choice" type="button" data-narrative-dice-choice="${escapeHtml(choice.id)}" aria-label="选择${escapeHtml(choice.label)}，${escapeHtml(effectText)}">
        <span>${escapeHtml(choice.label)}</span>
        <small>${escapeHtml(effectText)}</small>
      </button>
    `;
...
function renderMapActionDialog() {
  ...
  const actionButtons = selectedLocation.actionIds.map((actionId) => ACTION_BY_ID.get(actionId)).filter(Boolean).map((action) => `
    <button class="button map-action-button" type="button" data-map-action="${escapeHtml(action.id)}" ${actionDisabled ? "disabled" : ""} aria-label="在${escapeHtml(selectedLocation.name)}执行${escapeHtml(action.title)}，消耗 1 点行动力">
      <span>${escapeHtml(action.title)}<small>${escapeHtml(action.description)}</small></span>
      <span>耗费 1</span>
    </button>
  `).join("");
